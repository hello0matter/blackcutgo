        ...
        try:
            ...
            if self.path == '/base64':
                ...
                try:
                    ...
                    if len(text_data) == 5:
                        rf = text_data[4].strip("rf=")
                        re_zz = str(base64.b64decode(text_data[3][3:]).decode("utf-8"))  # 获取高级模式传递过来的正则内容
                        if rf == '0':
                            # 请求体数据
                            re_data = re.findall(re_zz, CAPTCHA)[0]
                            print("正则匹配结果：" + re_data)
                        elif rf == '1':
                            # 请求头数据
                            rp_head = re_zz.split("|")
                            head_key = rp_head[0]
                            re_zz = re_zz[len(head_key) + 1:]
